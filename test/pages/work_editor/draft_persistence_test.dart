...
void main() {
  test('首次提交依次创建、完整保存短篇正文、提交同一作品', () async {
    ...
    final result = await CreatorDraftPersistence(
      backend: backend,
    ).save(_draft(), languageId: 29, submitForReview: true);
  ...
  test('保存失败不提交，重试复用创建好的作品与修订 ID', () async {
    ...
    await expectLater(
      session.save(_draft(), languageId: 1, submitForReview: true),
  ...
  test('提交失败后重试使用最新乐观锁，且保持章节身份和顺序', () async {
    ...
    await expectLater(
      session.save(original, languageId: 1, submitForReview: true),
      throwsA(isA<CreatorDraftException>()),
    );
...
class _Backend extends CreatorDraftBackend {
  ...
  @override
  Future<ResultsType<Map<String, dynamic>>> submit(
    CreatorWorkDraft work,
  ) async {
    calls.add('submit');
    submitted.add(work);
