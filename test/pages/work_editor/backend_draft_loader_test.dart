...
void main() {
  ...
  test('恢复长篇独立章节顺序、内容及尚未完成的临时章节', () {
    ...
    data['chapters'] = [
      {'local_id': 'chapter_b', 'title': '第二章', 'content': '  内容 B\n', 'update_time': '2026-09-08 13:00:00'},
      {'chapter_uid': 'chapter_a', 'title': '第一章', 'content': '内容 A', 'update_time': 1788814800000},
    ];
    final work = creatorWorkDraftFromBackend(data);
    expect(work.chapters.map((chapter) => chapter.local_id), ['chapter_b', 'chapter_a']);
  ...
  test('章节数据或正文未完整读取时禁止以空内容打开编辑器', () {
    final data = _data();
    (data['draft'] as Map)['work_type'] = 1;
    expect(() => creatorWorkDraftFromBackend(data), throwsA(isA<CreatorDraftException>()));
    data['chapters'] = [{'local_id': 'chapter', 'title': '正文缺失'}];
    expect(() => creatorWorkDraftFromBackend(data), throwsA(isA<CreatorDraftException>()));
  });
...
Map<String, dynamic> _data() => {
  'novel': {'id': '12'},
  'draft': {
    'id': '23', 'novel_id': '12', 'novel_language_id': '34', 'language_id': '77',
    'lock_version': '9', 'revision_status': '1', 'work_type': '2', 'title': '草稿',
    'temp_chapter_content': '  正文\n尾部  ', 'temp_chapter_title': '未完成章节',
    'cover_url': 'https://example.com/cover.jpg', 'preferences': '{"2":["6"]}',
    'category_snapshot': '[{"category_id":"6"}]', 'saved_step': '9',
    'rights_confirmed': '1', 'release_mode': '1', 'update_time': '2026-09-08 12:00:00',
  },
};
